# Energy Trading Lab:面向能源交易的科研仿真实验智能体(中文版)
...
| 档位 | 时间窗 | RL 训练回合数 | 用途 |
| --- | --- | --- | --- |
| `quick` | 48 小时 | 100 | 面试现场快速展示,确认端到端链路 |
| `research` | 168 小时 | 3000 | 默认产品体验,真实生成脚本、执行子进程、训练 RL baseline / proposed method |
| `deep` | 336 小时 | 12000 | 更长时间泛化验证,可用于录制 Demo 或离线实验 |
...
也可以接 DeepSeek、Qwen、Kimi 等兼容 OpenAI `/chat/completions` 的服务,只需要替换 `ENERGY_LAB_LLM_BASE_URL` 和 `ENERGY_LAB_LLM_MODEL`。完整 provider 列表和 BYOK 优先级见 `docs/llm-adapters.md`。
...
完整目录导览见 `AGENTS.md`。三个插件式扩展面:

- `p2plab/algorithm_templates/<family>/<name>/`:加新算法,一个文件夹搞定,见 `docs/skills-protocol.md`。
- `scenarios/<grid>/`:加新电网场景,一个文件夹搞定,见 `docs/scenarios-protocol.md`。
- `p2plab/llm_adapters/`:加新 LLM provider,一个 Python 文件搞定,见 `docs/llm-adapters.md`。

## 协议 / 文档

- `AGENTS.md` — 目录导览 + 数据目录契约 + 能力暴露双轨约定
- `QUICKSTART.md` — 一页式本地启动
- `CONTRIBUTING.md` — "三个可在一个下午 ship 的事"
- `docs/spec.md` — 设计意图
- `docs/architecture.md` — 运行时拓扑 + 数据流
- `docs/roadmap.md` — 0.2.x → 1.0.0 路线图
